...
describe('Spore', () => {
  ...
  describe('Spore basics', () => {
    ...
    it('Transfer a Spore', async () => {
      ...
      if (hash) {
        ...
        if (existingSporeRecord) {
          existingSporeRecord = newRecord;
        } else {
          SPORE_OUTPOINT_RECORDS.push(newRecord);
        }
      }
    }, 0);
    it('Melt a Spore', async () => {
      ...
      if (!hash) {
        SPORE_OUTPOINT_RECORDS.push(sporeRecord);
      }
    }, 0);
  });

  describe('Spore (latest) with immortal mutant', () => {
    let existingSporeRecord: OutPointRecord | undefined;
    ...
    it('Try melt an immortal Spore', async () => {
      ...
      const sporeCell = await retryQuery(() => getSporeByOutPoint(existingSporeRecord!.outPoint, config));
  ...
  // TODO: Skipped because Mutant-related features are incomplete
